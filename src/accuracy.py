...
for type in postTypes:
    ...
    with open(file1Path) as file1, open(file2Path) as file2:
        for line in file1:
            data = json.loads(line)
            list1.append((data['labels'], type))
            textList1.append([data['text'], type + ".txt"])
        for line in file2:
            data = json.loads(line)
            list2.append((data['labels'], type + '_openAIGenerated.txt'))
...
resultsPath = parent_directory / "data" / 'results.txt'
with open(resultsPath, 'w', encoding = 'utf-8', errors='ignore') as output:
    for type in postTypes:
